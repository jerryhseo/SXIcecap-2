...
class DataTypeEditor extends React.Component {
	constructor(props) {
		...
		this.dataType = { dataTypeId: props.params.dataTypeId ?? 0 };
		this.structureLink = {};
		...
		this.fieldCommentable = Parameter.createParameter(
			this.namespace,
			this.formId,
			this.languageId,
			this.availableLanguageIds,
		...
		this.fieldVerifiable = Parameter.createParameter(
			this.namespace,
			this.formId,
			this.languageId,
			this.availableLanguageIds,
		...
		this.fieldFreezable = Parameter.createParameter(
			this.namespace,
			this.formId,
			this.languageId,
			this.availableLanguageIds,
		...
		this.optionGroup = Parameter.createParameter(
			...
			ParamType.GROUP,
			{
				...
				viewType: GroupParameter.ViewTypes.ARRANGEMENT,
				members: [this.fieldCommentable, this.fieldVerifiable, this.fieldFreezable],
				membersPerRow: 3
			}
		);
		...
		this.fields.push(this.optionGroup);
		...
		if (dataPacket.paramName === DataTypeProperty.NAME) {
			if (Util.isNotEmpty(this.parameterCode.value)) {
				Util.ajax({
					...
					successFunc: (result) => {
						if (!result) {
							this.parameterCode.setError(ErrorClass.ERROR, Util.translate("datatype-code-duplicated"));
			...
			Util.ajax({
				...
				successFunc: (result) => {
					...
					if (Util.isNotEmpty(result.dataStructure)) {
						this.dataStructure = result.dataStructure;
					}
